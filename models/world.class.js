class World {
  ...
  checkInventorySpace(bottle) {
    return (
      ...
      this.character.bottleInventoryCounter <= 4
    );
  }
  ...
  checkIfBossIsInVision() {
    return (
      !this.character.gamePaused &&
      this.character.x >= 2000 &&
      !this.level.endboss.denyCheck
    );
  }
  ...
  checkIfBossIsRightSide() {
    return (
      ...
      this.character.x + this.character.width / 2 <
        this.level.endboss.x + this.level.endboss.width / 2 - 600 &&
      this.level.endboss.bossRageStart
    );
  }
  ...
  checkBossDamageWorthy(bottle) {
    return (
      !this.gamePaused &&
      !this.level.endboss.isHurt(this.level.endboss.immortalDuration) &&
      this.level.endboss.bossRageStart &&
      !bottle.bottleHit &&
      !this.level.endboss.isDead() &&
  ...
  checkIfNormalChickenCanDoDamageToCharacter(enemy) {
    return (
      !this.gamePaused &&
      !this.character.isAboveNormalChicken() &&
      !this.character.isHurt(this.character.immortalDuration) &&
      enemy.energy > 0 &&
  ...
  checkIfSmallChickenCanDoDamageToCharacter(enemy) {
    return (
      !this.gamePaused &&
      !this.character.isAboveSmallChicken() &&
      !this.character.isHurt(this.character.immortalDuration) &&
      enemy.energy > 0 &&
      this.character.isCollidingHorizontal(enemy)
    );
  ...
  checkIfEndbossCanDoDamageToCharacter() {
    return (
      ...
      this.character.isCollidingHorizontal(this.level.endboss) &&
  ...
  checkIfCharacterHitsChickenFromTop(enemy) {
    return (
      !this.gamePaused &&
      !enemy.isDead() &&
      this.character.characterIsFalling() &&
      !this.character.isHurt(this.character.immortalDuration) &&
  ...
  checkIfBottleFlyRightDirection() {
    return (
      ...
      !this.character.otherDirection &&
      !this.character.isDead() &&
      this.character.bottleInventoryCounter > 0
    );
  }
