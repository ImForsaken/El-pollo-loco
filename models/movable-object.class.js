...
class MovableObject extends DrawableObject {
  ...
  isColliding(mo) {
    return (
      this.charRightCollideMoLeft(mo) &&
  ...
  isCollidingVerticaly(mo) {
    return (
      this.charBottomCollideMoTop(mo) && this.charTopCollideWithMoBottom(mo)
    );
  }
  ...
  playChickenMove(imagesWalking) {
    setInterval(() => {
      if (this.checkIfChickenIsMoving()) this.playAnimation(imagesWalking);
    }, 200);
  }
